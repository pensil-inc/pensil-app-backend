...
module.exports = class TeacherTimelineController {
    ...
    static async __invoke(req, res) {
        ...
        // get announcements
        batch.announcements = TeacherTimelineController.toTimelineObject((await Announcement.find({ batches: batch._id }).populate('owner')).map(announcement => ({
            id: announcement.id,
        ...
        // get materials
        batch.materials = TeacherTimelineController.toTimelineObject(new MaterialResource(await Material.find({ batch: batch._id }).populate('subject')), "material");
